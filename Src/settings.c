...
systemSettings_t systemSettings = { .ironTips[0 ... MAX_IRON_TIPS - 1] = {
		"DFLT", DFTL_PARAMS }, };

#define FLASH_ADDR (0x8000000|(0x10000-FLASH_PAGE_SIZE))/*Flash start OR'ed with the maximum amount of flash - 256 bytes*/
...
//	}
	uint16_t calc_crc16 = crc16_compute((uint8_t*) &systemSettings,
			offsetof(systemSettings_t, crc16), NULL);
	//resetSettings();
	if (systemSettings.version != SETTINGSVERSION
			|| calc_crc16 != systemSettings.crc16) {
		resetSettings();
		saveSettings();
	}
...
//	}
	int idx = 0;
	systemSettings.ironTips[idx++] = (tipData ) { "B", 1300, 2100, 2600, {
					0.0015, 0.002, 0, DF_LIMS } };
	systemSettings.ironTips[idx++] = (tipData ) { "BC2", 1200, 2000, 3100, {
					0.003, 0.0025, 0, DF_LIMS } };
	systemSettings.ironTips[idx++] = (tipData ) { "BL", 1100, 1800, 2800, {
					0.0015, 0.002, 0, DF_LIMS } };
	...
	systemSettings.ironTips[idx++] = (tipData ) { "C4", 1200, 2600, 2900, {
					0.002, 0.003, 0, DF_LIMS } };
	systemSettings.ironTips[idx++] = (tipData ) { "D16", 1200, 2000, 2600, {
					0.003, 0.003 * 2, 0, DF_LIMS } };
	systemSettings.ironTips[idx++] = (tipData ) { "D24", 1300, 2300, 3000, {
					0.003, 0.003 * 2, 0, DF_LIMS } };
	systemSettings.ironTips[idx++] = (tipData ) { "ILS", 1400, 2150, 2800, {
					0.001, 0.003, 0, DF_LIMS } };
	systemSettings.ironTips[idx++] = (tipData ) { "406", 1000, 2000, 3100, {
					0.001, 0.001, 0, DF_LIMS } };
	systemSettings.ironTips[idx++] = (tipData ) { "403", 1000, 2000, 3100, {
					0.001, 0.001, 0, DF_LIMS } };
